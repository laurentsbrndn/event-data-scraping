...
def get_lat_lng(venue_name, city="", province=""):
    ...
    # Nama venue yang sudah bersih dari spasi berantakan
    venue_name = clean_location_name(venue_name)
    v_lower = venue_name.lower()

    # 1. Cek dari dictionary alias
    ...
    # 3. Cek Nama venue secara langsung
    lat, lng = fetch_apple_maps(venue_name)
    if lat and lng:
        return lat, lng

    # 4. Ambil string sebelum tanda koma pertama
    if "," in venue_name:
    ...
    # 5. Fallback tambah kata "Indonesia" jika bukan di luar negeri
